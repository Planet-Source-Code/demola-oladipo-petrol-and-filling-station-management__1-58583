...
Begin VB.Form frmRetrenched2 
   ...
   End
End
Attribute VB_Name = "frmRetrenched2"
...
Private Sub Command1_Click()
    ...
    sSQL = "select * from tblRetrench2 where tblStaffID = '" & List1.List(Combo1.ListIndex) & "'"
    Set rsStaff = cn.Execute(sSQL)
    ...
    sSQL = "delete from tblretrench2 where tblStaffId = '" & List1.List(Combo1.ListIndex) & "'"
    cn.Execute sSQL
...
Sub loadID()
    sSQL = "select tblStaffID, tblSurname, tblFirstname from tblRetrench2"
    Set rsStaff = cn.Execute(sSQL)
    Combo1.Clear
    List1.Clear
    With rsStaff
